...
//    TH2F *h2 = (TH2F*)(f->Get(Form("xtal_r_vs_tote_x#%d_s#%d",xtal,segnum)));
    TH2F *h2 = (TH2F*)(f->Get(Form("sse2/xtal_r_vs_sse2_x#%d_s#%d",xtal,segnum)));
    if(!h2) return;
    h2->SetName("h2");
    h2->RebinX(4);
...
void gt_neutronfix_Nov24()
{
    ...
    for(seg=1; seg<6; seg++) {
//        int xtal = 34;
//        oneline("../output.root", 898, 880, 915,25,xtal,seg);
//        oneline("../output.root", 1836, 1800, 1885,25,xtal,seg);
